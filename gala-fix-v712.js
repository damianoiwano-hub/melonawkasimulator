...
globalThis.MELON_UI_VERSION='V7.13';
const AUTO='melon_auto_v5',STORE='melon_gala_categories_v713',MANIFEST='./data/database.json';
...
function scoreFor(name,cat,s,year){const p=profile(name,s,year),id=cat.id;let v=50;
 ...
 const stableNoise=.9+unit(`${year}|${cat.id}|${name}|base`)*.2;if(key(name)===key(s.nickname))v*=activityPower(s);return Math.max(8,v*stableNoise)
}
...
function buildAwards(s,year){const root=json(STORE,{}),cid=careerId(s);root[cid]=root[cid]||{};if(root[cid][year]?.schema===713)return root[cid][year];const pool=candidatePool(s),awards=[],usedWinners=new Set();if(pool.length<3)return{schema:713,year,awards:[],error:'Za mało kandydatów'};for(const cat of categories){const eligible=pool.filter(n=>!usedWinners.has(key(n)));if(eligible.length<3)break;const nominees=weightedSample(eligible,3,n=>scoreFor(n,cat,s,year)),results=normalizedResults(nominees,cat,s,year),winner=results[0]?.name||'';if(winner)usedWinners.add(key(winner));awards.push({...cat,nominees:results.map(r=>r.name),winner,results})}root[cid][year]={schema:713,year,generatedAt:new Date().toISOString(),player:s.nickname,playerIncluded:true,activityPower:activityPower(s),uniqueWinners:true,winners:[...usedWinners],awards};localStorage.setItem(STORE,JSON.stringify(root));return root[cid][year]}
function playerNotice(s){return`<div class="gala-player-notice-v712"><span>🎮 AKTYWNY GRACZ</span><strong>${esc(s.nickname||'Użytkownik')}</strong><small>Bierzesz udział automatycznie. Aktywność, poziom i seria zwiększają Twoją wagę. Jedna osoba może wygrać maksymalnie jedną kategorię podczas tej samej gali.</small></div>`}
